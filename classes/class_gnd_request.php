...
class gnd_request {
    ...
    public function __toString() {
        $parts = array($this->preferredName);
        if ($this->dateBirth or $this->dateDeath) {
            $parts[] = $this->dateBirth.'–'.$this->dateDeath;
        }
        if ($this->academicDegree) {
            $parts[] = $this->academicDegree;
        }
        if ($this->placeBirth) {
            $parts[] = 'Geburtsort: '.$this->placeBirth;
        }
        if ($this->placeDeath) {
            $parts[] = 'Sterbeort: '.$this->placeDeath;
        }
        if ($this->placesActivity) {
            $parts[] = 'Wirkungsort(e): '.implode(', ', $this->placesActivity);
        }
        /*if ($this->info) {
            $parts[] = 'Info: '.$this->info;
        }*/
